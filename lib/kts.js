...
Client.prototype.request = function (action, param, callback) {
  try {
    ...
    err = setError(-1, 'parseParamError', err);
    if (callback)
      return callback(err);
    throw err;
  }
  ...
  var req = http.request(http_options, function (res) {
    res.on("error", function (err) {
      err = setError(-2, 'requestFailed', err);
      if (callback)
        return callback(err);
      throw err;
    });
    ...
    res.on("end", function () {
      try {
        ...
        // body not a pb string, try other decode
        err = setError(-3, 'apiError', err);
      ...
      try {
        ...
        // error code must set in proto api
        if (callback)
          return callback(err);
        throw err;
      }
  ...
  req.on("error", function (err) {
    err = setError(-2, 'requestError', err);
    if (callback)
      return callback(err);
    throw err;
  });
